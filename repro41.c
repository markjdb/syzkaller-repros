Syzkaller hit 'panic: MSan: Uninitialized UMA memory in bufwrite, offset 54/NUM, addr ADDR, from namei+0xa2a' bug.

login: panic: MSan: Uninitialized UMA memory in bufwrite, offset 54/32768, addr 0xfffffe00146e8036, from namei+0xa2a
cpuid = 0
time = 1628393005
KDB: stack backtrace:
db_trace_self_wrapper() at db_trace_self_wrapper+0xde/frame 0xfffffe001a74ea90
vpanic() at vpanic+0x639/frame 0xfffffe001a74eb50
panic() at panic+0x1b3/frame 0xfffffe001a74ec60
kmsan_report_hook() at kmsan_report_hook+0x2e8/frame 0xfffffe001a74ede0
bufwrite() at bufwrite+0x8ea/frame 0xfffffe001a74ee90
ffs_balloc_ufs2() at ffs_balloc_ufs2+0x344e/frame 0xfffffe001a74f1d0
ffs_truncate() at ffs_truncate+0x145f/frame 0xfffffe001a74f4f0
ufs_setattr() at ufs_setattr+0x17d4/frame 0xfffffe001a74f690
VOP_SETATTR_APV() at VOP_SETATTR_APV+0x15e/frame 0xfffffe001a74f730
vn_truncate_locked() at vn_truncate_locked+0x37e/frame 0xfffffe001a74f890
vn_truncate() at vn_truncate+0x765/frame 0xfffffe001a74f980
kern_ftruncate() at kern_ftruncate+0x58d/frame 0xfffffe001a74fa10
amd64_syscall() at amd64_syscall+0xa4e/frame 0xfffffe001a74fbf0
fast_syscall_common() at fast_syscall_common+0xf8/frame 0xfffffe001a74fbf0
--- syscall (0, FreeBSD ELF64, nosys), rip = 0x249dfa, rsp = 0x7fffffffe9e8, rbp = 0x7fffffffe9f0 ---
KDB: enter: panic
[ thread pid 824 tid 100107 ]
...
rbx                          0
rsp         0xfffffe001a74ea30
rbp         0xfffffe001a74ea90
rsi                        0x1
...
db> show proc
Process 824 (syz-executor0332980) at 0xfffffe003b34b538:
 state: NORMAL
 uid: 0  gids: 0, 0, 5
 parent: pid 822 at 0xfffffe003b3a2a70
 ABI: FreeBSD ELF64
 flag: 0x10000000  flag2: 0
 arguments: /root/syz-executor033298052
 reaper: 0xfffffe0011387538 reapsubtree: 1
 sigparent: 20
 vmspace: 0xfffffe001f9ac9f0
   (map 0xfffffe001f9ac9f0)
   (map.pmap 0xfffffe001f9acab0)
   (pmap 0xfffffe001f9acb10)
 threads: 1
100107                   Run     CPU 0                       syz-executor0332980
db> ps
  pid  ppid  pgrp   uid  state   wmesg   wchan               cmd
  824   822   820     0  R       CPU 0                       syz-executor0332980
  822   820   820     0  S       nanslp  0xffffffff86ff77e1  syz-executor0332980
  820   818   820     0  Ss      pause   0xfffffe003b34b0b0  csh
  818   735   818     0  Ss      select  0xfffffe003ae49cc0  sshd
  797     1   797     0  Ss+     ttyin   0xfffffe001aaf8cb0  getty
  793     1   793     0  Ss      select  0xfffffe0039c430c0  logger
  787     1    22     0  S+      piperd  0xfffffe003ae692e8  logger
  786   785    22     0  S+      nanslp  0xffffffff86ff77e0  sleep
  785     1    22     0  S+      wait    0xfffffe003a909000  sh
  781     1   781    25  Ss      pause   0xfffffe0039bfc0b0  sendmail
  778     1   778     0  Ss      select  0xfffffe001aad5dc0  sendmail
  739     1   739     0  Ss      nanslp  0xffffffff86ff77e0  cron
  735     1   735     0  Ss      select  0xfffffe001aad5d40  sshd
  646     1   646     0  Ss      select  0xfffffe0039c43e40  syslogd
  449     1   449     0  Ss      select  0xfffffe0039c43ec0  devd
  352     1   352    65  Ss      select  0xfffffe001aad5840  dhclient
  270     1   270     0  Ss      select  0xfffffe0039c43140  dhclient
...
100086                   D       sdflush 0xfffffe0039a4bce8  [/ worker]
...
db> show all locks
Process 824 (syz-executor0332980) thread 0xfffffe003ae16740 (100107)
exclusive lockmgr bufwait (bufwait) r = 0 (0xfffffe0010c49620) locked @ /root/freebsd/sys/kern/vfs_bio.c:3940
exclusive lockmgr ufs (ufs) r = 0 (0xfffffe003af3d5b0) locked @ /root/freebsd/sys/kern/vfs_vnops.c:1612
db> show malloc
         ...
         sysctloid        30416         1795K        30458
            ...
            linker          227         1492K          240
              kobj          323         1292K          483
            newblk          594         1173K          681
            devbuf         1360         1150K         1377
          vfscache            3         1025K            3
               pcb           16          777K           32
          inodedep           67          537K          108
         ufs_quota            1          512K            1
          vfs_hash            1          512K            1
           callout            2          512K            2
              intr            4          472K            4
...
db> show uma
   ...
   mbuf_jumbo_page   4096    1088     950       14748       0     254    8347648        0
      malloc-16384  16384     110       0         110       0       1    1802240        0
       malloc-4096   4096     325       1         485       0       2    1335296        0
          BUF TRIE    144     174    8982         467       0      62    1318464        0
        malloc-128    128   10055      51       10150       0     126    1293568        0
              pbuf   2624       0     264           0       0       2     692736        0
              mbuf    256    1346    1024       22911       0     254     606720        0
        malloc-512    512    1028      12        1219       0      30     532480        0
      malloc-65536  65536       6       1         106       0       1     458752        0
         vmem btag     56    7281      38        7281       0     254     409864        0
        RADIX NODE    144    2387     435       29033       0      62     406368        0
            lkpimm    168       1    2327           1       0      62     391104        0
          lkpicurr    168       2    2326           2       0      62     391104        0
       UMA Slabs 0    112    3020       1        3020       0     126     338352        0
       malloc-2048   2048     130       4         175       0       8     274432        0
         VM OBJECT    264     928      47       12118       0      30     257400        0
             VNODE    448     526      41         558       0      30     254016        0
         malloc-64     64    3595      59        4427       0     254     233856        0
        256 Bucket   2048      97      17        2864       0       8     233472        0
        malloc-256    256     803      37        1628       0      62     215040        0
       malloc-4096   4096      48       2        1141       0       2     204800        0
         malloc-16     16   12648     102       12698       0     254     204000        0
            THREAD   1808     109       1         109       0       8     198880        0
            DEVCTL   1024       0     192          93       0       0     196608        0
         UMA Zones    768     222       2         222       0      16     172032        0
         malloc-32     32    4456     206        4555       0     254     149184        0
       FFS2 dinode    256     505      20         534       0      62     134400        0
      malloc-65536  65536       2       0           2       0       1     131072        0
      malloc-65536  65536       2       0           2       0       1     131072        0
       S VFS Cache    104     999     132        1081       0     126     117624        0
          ksiginfo    112      37    1007          69       0     126     116928        0
       malloc-1024   1024     102      10        1147       0      16     114688        0
         FFS inode    192     505      41         534       0      62     104832        0
         MAP ENTRY     96     804     288       37978       0     126     104832        0
      malloc-32768  32768       0       3           4       0       1      98304        0
      malloc-32768  32768       3       0           3       0       1      98304        0
        malloc-128    128     653      60        1764       0     126      91264        0
       malloc-8192   8192       8       3          82       0       1      90112        0
          UMA Kegs    384     207       6         207       0      30      81792        0
              PROC   1336      40      14         824       0       8      72144        0
        malloc-128    128     495      63        3035       0     126      71424        0
           VMSPACE   2544      19       8         804       0       4      68688        0
      mbuf_cluster   2048      30       2          30       0     254      65536        0
      malloc-65536  65536       1       0           1       0       1      65536        0
      malloc-65536  65536       1       0           1       0       1      65536        0
      malloc-32768  32768       1       1           2       0       1      65536        0
      malloc-32768  32768       2       0           2       0       1      65536        0
        128 Bucket   1024      45      14         677       0      16      60416        0
         filedesc0   1072      41      15         825       0       8      60032        0
        malloc-256    256     205      20         524       0      62      57600        0
      ...
      malloc-16384  16384       3       0           3       0       1      49152        0
        ...
        malloc-128    128     260      50       28673       0     126      39680        0
         ...
         32 Bucket    256      39     111         508       0      62      38400        0
             NAMEI   1024       0      36       12423       0      16      36864        0
        malloc-384    384      70      20         119       0      30      34560        0
      ...
      malloc-32768  32768       0       1          53       0       1      32768        0
       malloc-8192   8192       4       0           4       0       1      32768        0
         ...
         64 Bucket    512      48       8         719       0      30      28672        0
        malloc-128    128     162      55         220       0     126      27776        0
             g_bio    384       0      70        4805       0      30      26880        0
      ...
      malloc-16384  16384       1       0           1       0       1      16384        0
       ...
       malloc-8192   8192       0       2           2       0       1      16384        0
       ...
       malloc-1024   1024       3       5           3       0      16       8192        0
        malloc-512    512       8       8           9       0      30       8192        0
          ...
          rl_entry     40      30     172          32       0     254       8080        0
             udpcb     32       2     250          60       0     254       8064        0
               PWD     32      13     239          98       0     254       8064        0
         malloc-64     64      50      76        8570       0     254       8064        0
         malloc-64     64     116      10         145       0     254       8064        0
         malloc-64     64       4     122          39       0     254       8064        0
         malloc-64     64       3     123           7       0     254       8064        0
         malloc-32     32      29     223         173       0     254       8064        0
         ...
         16 Bucket    144      30      26         244       0      62       8064        0
          4 Bucket     48       5     163          47       0     254       8064        0
          2 Bucket     32      44     208         581       0     254       8064        0
         ...
         malloc-16     16       2     498         126       0     254       8000        0
          8 Bucket     80      29      71         276       0     126       8000        0
         tcp_inpcb    496       4      12          10       0     254       7936        0
        malloc-128    128      18      44         223       0     126       7936        0
     ...
     routing nhops    256       8      22          14       0      62       7680        0
       mbuf_packet    256       0      30         101       0     254       7680        0
        ...
        malloc-256    256       2      28         316       0      62       7680        0
 ...
 epoch_record pcpu    256       4      12           4       0      62       4096        0
      ...
      vtnet_tx_hdr     24       1     166        7084       0     254       4008        0
...
r0 = open(&(0x7f0000000080)='./file0\x00', 0x282, 0x0)
ftruncate(r0, 0x7e8000000000)
freebsd11_mknod(&(0x7f0000000000)='./file1\x00', 0xd9609053aebe99e, 0x2000103)
...
void execute_one(void)
{
  ...
  if (res != -1)
    r[0] = res;
  syscall(SYS_ftruncate, r[0], 0x7e8000000000ul);
  memcpy((void*)0x20000000, "./file1\000", 8);
  syscall(SYS_freebsd11_mknod, 0x20000000ul, 0xd9609053aebe99eul, 0x2000103);
}
int main(void)
{
