Syzkaller hit 'panic: pmap_kmsan_enter_alloc_4k: no memory to grow shadow map' bug.

login: panic: pmap_kmsan_enter_alloc_4k: no memory to grow shadow map
cpuid = 0
time = 1627004019
KDB: stack backtrace:
db_trace_self_wrapper() at db_trace_self_wrapper+0xde/frame 0xfffffe001a7bfe70
vpanic() at vpanic+0x639/frame 0xfffffe001a7bff30
panic() at panic+0x1b3/frame 0xfffffe001a7c0040
pmap_kmsan_enter() at pmap_kmsan_enter+0x1494/frame 0xfffffe001a7c00e0
kmsan_shadow_map() at kmsan_shadow_map+0x58/frame 0xfffffe001a7c0110
pmap_growkernel() at pmap_growkernel+0x28a/frame 0xfffffe001a7c01a0
vm_map_insert() at vm_map_insert+0x1368/frame 0xfffffe001a7c0370
vm_map_find() at vm_map_find+0x21d1/frame 0xfffffe001a7c0530
kva_import() at kva_import+0x15c/frame 0xfffffe001a7c05c0
vmem_try_fetch() at vmem_try_fetch+0x54e/frame 0xfffffe001a7c0680
vmem_xalloc() at vmem_xalloc+0x3917/frame 0xfffffe001a7c0820
kva_import_domain() at kva_import_domain+0x14e/frame 0xfffffe001a7c08c0
vmem_try_fetch() at vmem_try_fetch+0x54e/frame 0xfffffe001a7c0980
vmem_xalloc() at vmem_xalloc+0x3917/frame 0xfffffe001a7c0b20
vmem_alloc() at vmem_alloc+0x3a5/frame 0xfffffe001a7c0be0
kmem_malloc_domainset() at kmem_malloc_domainset+0x26d/frame 0xfffffe001a7c0ca0
malloc_large() at malloc_large+0xdf/frame 0xfffffe001a7c0d40
malloc() at malloc+0x51e/frame 0xfffffe001a7c0e10
pfioctl() at pfioctl+0x20f8/frame 0xfffffe001a7c1400
devfs_ioctl() at devfs_ioctl+0x4b9/frame 0xfffffe001a7c14d0
VOP_IOCTL_APV() at VOP_IOCTL_APV+0x13c/frame 0xfffffe001a7c1560
vn_ioctl() at vn_ioctl+0xa30/frame 0xfffffe001a7c1710
devfs_ioctl_f() at devfs_ioctl_f+0x18a/frame 0xfffffe001a7c17d0
kern_ioctl() at kern_ioctl+0xee3/frame 0xfffffe001a7c18d0
sys_ioctl() at sys_ioctl+0xa72/frame 0xfffffe001a7c1a10
amd64_syscall() at amd64_syscall+0xa4e/frame 0xfffffe001a7c1bf0
fast_syscall_common() at fast_syscall_common+0x104/frame 0xfffffe001a7c1bf0
--- syscall (0, FreeBSD ELF64, nosys), rip = 0x2332da, rsp = 0x7fffffffea68, rbp = 0x7fffffffea80 ---
KDB: enter: panic
[ thread pid 822 tid 100092 ]
...
gs                        0x1b
ss                        0x28
rax         0xffffffff8632a74e
rcx                          0
rdx         0xffffffff8702b000  msan_dummy_orig
rbx                          0
rsp         0xfffffe001a7bfe10
rbp         0xfffffe001a7bfe70
rsi                        0x1
...
r11                  0x6bb8dc0
r12                          0
r13         0xfffffe005c58af01
r14         0xfffffe005c58afb8
r15         0xffffffff8632a74e
rip         0xffffffff83f95fc7  kdb_enter+0x1c7
rflags                    0x46
kdb_enter+0x1c7:        movq    $0x87024010,%rdi
db> show proc
Process 822 (syz-executor4922407) at 0xfffffe005c579538:
 state: NORMAL
 uid: 0  gids: 0, 0, 5
 parent: pid 820 at 0xfffffe0045f65538
 ABI: FreeBSD ELF64
 flag: 0x10004000  flag2: 0
 arguments: /root/syz-executor492240738
 reaper: 0xfffffe0011383538 reapsubtree: 1
 sigparent: 20
 vmspace: 0xfffffe0045f6c3e0
   (map 0xfffffe0045f6c3e0)
   (map.pmap 0xfffffe0045f6c4a0)
   (pmap 0xfffffe0045f6c500)
 threads: 1
100092                   Run     CPU 0                       syz-executor4922407
db> ps
  pid  ppid  pgrp   uid  state   wmesg   wchan               cmd
  822   820   820     0  R       CPU 0                       syz-executor4922407
  820   818   820     0  SWs     pause   0xfffffe0045f655e8  csh
  818   781   818     0  Ss      select  0xfffffe005c5b0ac0  sshd
  797     1   797     0  Ss+     ttyin   0xfffffe001aae8cb0  getty
  794     1   794     0  Ss      select  0xfffffe001a8f9240  logger
  788     1    20     0  S+      piperd  0xfffffe005c5d35d0  logger
  787   786    20     0  SW+     nanslp  0xffffffff86ff77a1  sleep
  786     1    20     0  SW+     wait    0xfffffe005c59c538  sh
  781     1   781     0  Ss      select  0xfffffe005c5b0cc0  sshd
  758     1   758    25  SWs     pause   0xfffffe005c59c0b0  sendmail
  755     1   755     0  Ss      select  0xfffffe001a907cc0  sendmail
  726     1   726     0  SWs     nanslp  0xffffffff86ff77a1  cron
  646     1   646     0  Ss      select  0xfffffe001a9210c0  syslogd
  448     1   448     0  Ss      select  0xfffffe001a921140  devd
  352     1   352    65  Ss      select  0xfffffe005c5b0ec0  dhclient
  270     1   270     0  Ss      select  0xfffffe001a921340  dhclient
  267     1   267     0  Ss      select  0xfffffe001a9213c0  dhclient
  102     0     0     0  DL      pftm    0xffffffff87731b30  [pf purge]
   33     0     0     0  DL      syncer  0xffffffff870ee7d0  [syncer]
   32     0     0     0  DL      vlruwt  0xfffffe0045f2aa70  [vnlru]
   19     0     0     0  DL      (threaded)                  [bufdaemon]
100069                   D       qsleep  0xffffffff870ed8a0  [bufdaemon]
100070                   D       -       0xffffffff86a0af80  [bufspacedaemon-0]
100081                   D       sdflush 0xfffffe001a9034e8  [/ worker]
   18     0     0     0  DL      psleep  0xffffffff87113548  [vmdaemon]
   17     0     0     0  RL      (threaded)                  [pagedaemon]
100067                   D       pwait   0xffffffff866d2c10  [dom0]
100071                   D       laundp  0xffffffff866d2c10  [laundry: dom0]
100072                   Run     CPU 1                       [uma]
...
100059                   D       -       0xffffffff866d2c11  [deadlkres]
...
db> show all locks
Process 822 (syz-executor4922407) thread 0xfffffe005c07dc80 (100092)
exclusive sleep mutex vm map (system) (vm map (system)) r = 0 (0xffffffff87106f88) locked @ /root/freebsd/sys/vm/vm_map.c:2127
Process 17 (pagedaemon) thread 0xfffffe001a8e1000 (100072)
exclusive sleep mutex pmap (pmap) r = 0 (0xffffffff87140c30) locked @ /root/freebsd/sys/amd64/amd64/pmap.c:6258
shared rw UMA lock (UMA lock) r = 0 (0xffffffff86a445c0) locked @ /root/freebsd/sys/vm/uma_core.c:3084
shared sx umareclaim (umareclaim) r = 0 (0xffffffff87106e30) locked @ /root/freebsd/sys/vm/uma_core.c:5198
db> show malloc
...
db> show uma
   ...
   mbuf_jumbo_page   4096    1088       0       14472       0     126    4456448        0
      ...
      malloc-16384  16384     105       0         105       0       1    1720320        0
       ...
       malloc-4096   4096     323       1         483       0       2    1327104        0
          BUF TRIE    144     176    8980         443       0      62    1318464        0
              mbuf    256    1346    1054       22362       0     254     614400        0
        malloc-512    512    1026      14        1034       0      30     532480        0
      malloc-65536  65536       6       1          37       0       1     458752        0
         vmem btag     56    7577     606        8050       0     254     458248        0
        RADIX NODE    144    2378     388       28449       0      62     398304        0
            lkpimm    160       1    2324           1       0      62     372000        0
          lkpicurr    160       2    2323           2       0      62     372000        0
       UMA Slabs 0    112    2521     500        3098       0     126     338352        0
       malloc-2048   2048     136       6         160       0       8     290816        0
         VM OBJECT    264     904      71       11700       0      30     257400        0
             VNODE    448     524      34         554       0      30     249984        0
        256 Bucket   2048      74      44        2978       0       8     241664        0
      ...
      malloc-65536  65536       2       0           2       0       1     131072        0
       malloc-8192   8192      12       2          15       0       1     114688        0
       S VFS Cache    104     992     100        1072       0     126     113568        0
       malloc-1024   1024     101       7         229       0      16     110592        0
         MAP ENTRY     96     776     358       36958       0     126     108864        0
         FFS inode    192     502      44         529       0      62     104832        0
            DEVCTL   1024       0     100          93       0       0     102400        0
      malloc-32768  32768       2       1           4       0       1      98304        0
      mbuf_cluster   2048      45       1          45       0     130      94208        0
            clpbuf   2624       0      32          27       0       9      83968        0
          UMA Kegs    384     207       6         207       0      30      81792        0
           VMSPACE   2544      18       9         802       0       4      68688        0
         malloc-64     64     984      87       16912       0     254      68544        0
      malloc-32768  32768       2       0          54       0       1      65536        0
              PROC   1336      39       9         822       0       8      64128        0
        malloc-256    256     224      16         471       0      62      61440        0
        128 Bucket   1024      33      26        4989       0      16      60416        0
        malloc-384    384     111      29         150       0      30      53760        0
         filedesc0   1072      40       9         823       0       8      52528        0
        malloc-128    128     374      29        3640       0     126      51584        0
      ...
      malloc-16384  16384       1       0          65       0       1      16384        0
       ...
       malloc-4096   4096       4       0           5       0       2      16384        0
             tcpcb   1080       4      10          10       0     129      15120        0
       ...
       malloc-1024   1024       9       3           9       0      16      12288        0
        ...
        malloc-128    128      43      50         161       0     126      11904        0
             unpcb    256      12      33         931       0     156      11520        0
       ...
       malloc-1024   1024       4       4           4       0      16       8192        0
        ...
        malloc-512    512       3      13          40       0      30       8192        0
         ...
         16 Bucket    144      25      31        1541       0      62       8064        0
          4 Bucket     48       2     166          52       0     254       8064        0
          2 Bucket     32      13     239         556       0     254       8064        0
         malloc-16     16      21     479         176       0     254       8000        0
         malloc-16     16      10     490          53       0     254       8000        0
         ...
         malloc-16     16       7     493           7       0     254       8000        0
          8 Bucket     80      17      83         310       0     126       8000        0
         tcp_inpcb    496       4      12          10       0     140       7936        0
        malloc-128    128      15      47          21       0     126       7936        0
 ...
 epoch_record pcpu    256       4      12           4       0      62       4096        0
      ...
      vtnet_tx_hdr     24       1     166        6896       0     254       4008        0
            ttyinq    160      15      10          45       0      62       4000        0
             ripcb    496       1       7           1       0     140       3968        0
         udp_inpcb    496       2       6          60       0     140       3968        0
        malloc-128    128       1      30           8       0     126       3968        0
       UMA Slabs 1    176       6      16           6       0      62       3872        0
     routing nhops    256       8       7          14       0      62       3840        0
...
pf table entry counters     64       0       0           0       0     254          0        0
   pf source nodes    136       0       0           0       0     156          0        0
     pf state keys     88       0       0           0       0     126          0        0
         pf states    312       0       0           0       0     140          0        0
     ...
     udplite_inpcb    496       0       0           0       0     140          0        0
      tcp_log_node    120       0       0           0       0     126          0        0
    tcp_log_bucket    176       0       0           0       0      62          0        0
           tcp_log    416       0       0           0       0     140          0        0
          tcpreass     48       0       0           0       0     254          0        0
tfo_ccache_entries     80       0       0           0       0     126          0        0
               tfo      4       0       0           0       0     254          0        0
          sackhole     32       0       0           0       0     254          0        0
          syncache    168       0       0           5       0     156          0        0
             tcptw     88       0       0           0       0     188          0        0
    ...
    IPsec SA lft_c     16       0       0           0       0     254          0        0
        ...
        TMPFS node    224       0       0           0       0      62          0        0
           NCLNODE    584       0       0           0       0      16          0        0
             NAMEI   1024       0       0       12369       0      16          0        0
     LTS VFS Cache    360       0       0           0       0      30          0        0
     STS VFS Cache    144       0       0           0       0      62          0        0
           cryptop    280       0       0           0       0      30          0        0
  linux_dma_object     24       0       0           0       0     254          0        0
  linux_dma_pctrie    144       0       0           0       0      62          0        0
             g_bio    384       0       0        4691       0      30          0        0
   IOMMU_MAP_ENTRY    120       0       0           0       0     126          0        0
      ktls_session    192       0       0           0       0      62          0        0
    mbuf_jumbo_16k  16384       0       0           0       0     125          0        0
     mbuf_jumbo_9k   9216       0       0           0       0     125          0        0
      audit_record   1280       0       0           0       0       8          0        0
         domainset     40       0       0           0       0     254          0        0
        MAC labels     40       0       0           0       0     254          0        0
            vnpbuf   2624       0       0           0       0      33          0        0
            mdpbuf   2624       0       0           0       0       3          0        0
           nfspbuf   2624       0       0           0       0       9          0        0
            swwbuf   2624       0       0           0       0       5          0        0
            swrbuf   2624       0       0           0       0       9          0        0
...
r0 = openat$ptmx(0xffffffffffffff9c, &(0x7f0000001940), 0x210901, 0x0)
ioctl$DIOCGETSTATES(r0, 0xc0104419, &(0x7f00000001c0)="")
...
int main(void)
{
  ...
  intptr_t res = 0;
  memcpy((void*)0x20001940, "/dev/pf\000", 8);
  res =
      syscall(SYS_openat, 0xffffffffffffff9cul, 0x20001940ul, 0x210901ul, 0ul);
  if (res != -1)
    r[0] = res;
  memcpy((void*)0x200001c0,
         "\x1a\xad\x12\x77\x70\x40\x24\xb4\x2a\x79\xb0\x59\x60\x6a\xb8\x2c\x55"
         "\x01\xce\x07\xf3\x3b\x71\x5b\xe8\x40\x28\x0b\xee\xcd\x97\xcb\x73\x7f"
         "\x8d\x2f\x5e\x06\xc0\x7e\xf3\xd6\x93\x97\x69\x01\x0f\x6d\xa8\x83\xb3"
         "\xa6\x64\xfd\x12\xe4\x58\x05\x8a\xc0\x61\x07\x5f\x69\x39\x2b\xa5\x3c"
         "\x97\x72\x5a\xf2\x50\x64\xde\x4e\x91\x80\xcc\x28\xdc\xe3\x07\x4d\x0d"
         "\xee\x40\xa5\x71\x88\x49\x46\xa6\xc6\xd5\x59\xdd\x2b\x4f\x88\x3f\xfa"
         "\xf1\xa1\x65\x15\x7a\xd2\xa9\x64\x77\x1e\xcf\x85\x7f\xec\x96\x36\xaf"
         "\x4a\x70\x19\x04\x8e\x05\xa5\xc6\x3f\xf2\x05\xb7\xa6\x48\x47\x5d\xb6"
         "\x34\x73\xa0\xff\x2d\x56\xc2\xd9\x55\x42\x7e\xe5\x99\xe2\xe4\xa5\xc1"
         "\xd5\xf9\xbc\x12\xb9\xcf\x49\x08\xaf\x90\xe9\x76\x22\x74\x04\x66\xac"
         "\x1b\x86\x90\x24\x3c\xd3\xe5\xee\x55\xc9\x93\xa2\x88\x5c\xd0\x9f\x52"
         "\x67\x24\xa4\xaf\x97\xb9\x3e\x0a\x76\xd7\x29\x74",
         199);
  syscall(SYS_ioctl, r[0], 0xc0104419ul, 0x200001c0ul);
  return 0;
}
