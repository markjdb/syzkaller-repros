Syzkaller hit 'panic: KMSAN: Uninitialized malloc memory from cod_alloc+0x354' bug.

login: panic: KMSAN: Uninitialized malloc memory from cod_alloc+0x354
cpuid = 0
time = 1627277406
KDB: stack backtrace:
db_trace_self_wrapper() at db_trace_self_wrapper+0xde/frame 0xfffffe001fd65ce0
vpanic() at vpanic+0x639/frame 0xfffffe001fd65da0
panic() at panic+0x1b3/frame 0xfffffe001fd65eb0
__msan_warning() at __msan_warning+0x264/frame 0xfffffe001fd66000
swcr_eta() at swcr_eta+0x1f7/frame 0xfffffe001fd66060
swcr_process() at swcr_process+0x115/frame 0xfffffe001fd660d0
crypto_invoke() at crypto_invoke+0x58b/frame 0xfffffe001fd661b0
crypto_dispatch() at crypto_dispatch+0x487/frame 0xfffffe001fd66210
crypto_ioctl() at crypto_ioctl+0xb284/frame 0xfffffe001fd66400
devfs_ioctl() at devfs_ioctl+0x4b9/frame 0xfffffe001fd664d0
VOP_IOCTL_APV() at VOP_IOCTL_APV+0x13c/frame 0xfffffe001fd66560
vn_ioctl() at vn_ioctl+0xa30/frame 0xfffffe001fd66710
devfs_ioctl_f() at devfs_ioctl_f+0x18a/frame 0xfffffe001fd667d0
kern_ioctl() at kern_ioctl+0xee3/frame 0xfffffe001fd668d0
sys_ioctl() at sys_ioctl+0xa72/frame 0xfffffe001fd66a10
amd64_syscall() at amd64_syscall+0xa4e/frame 0xfffffe001fd66bf0
fast_syscall_common() at fast_syscall_common+0x104/frame 0xfffffe001fd66bf0
--- syscall (0, FreeBSD ELF64, nosys), rip = 0x23339a, rsp = 0x7fffffffea68, rbp = 0x7fffffffea80 ---
KDB: enter: panic
[ thread pid 824 tid 100077 ]
...
rbx                          0
rsp         0xfffffe001fd65c80
rbp         0xfffffe001fd65ce0
rsi                        0x1
...
db> show proc
Process 824 (syz-executor9159705) at 0xfffffe001f974a70:
 state: NORMAL
 uid: 0  gids: 0, 0, 5
 parent: pid 822 at 0xfffffe001f974538
 ABI: FreeBSD ELF64
 flag: 0x10004000  flag2: 0
 arguments: /root/syz-executor915970530
 reaper: 0xfffffe0011383538 reapsubtree: 1
 sigparent: 20
 vmspace: 0xfffffe0039bc63e0
   (map 0xfffffe0039bc63e0)
   (map.pmap 0xfffffe0039bc64a0)
   (pmap 0xfffffe0039bc6500)
 threads: 1
100077                   Run     CPU 0                       syz-executor9159705
db> ps
  pid  ppid  pgrp   uid  state   wmesg   wchan               cmd
  824   822   822     0  R       CPU 0                       syz-executor9159705
  822   820   822     0  Ss      pause   0xfffffe001f9745e8  csh
  820   781   820     0  Ss      select  0xfffffe001137d4c0  sshd
  797     1   797     0  Ss+     ttyin   0xfffffe001aadacb0  getty
  794     1   794     0  Ss      select  0xfffffe001a96b040  logger
  788     1    22     0  S+      piperd  0xfffffe003ae322e8  logger
  787   786    22     0  S+      nanslp  0xffffffff86ff77a1  sleep
  786     1    22     0  S+      wait    0xfffffe003ae4d000  sh
  781     1   781     0  Ss      select  0xfffffe001a96b0c0  sshd
  758     1   758    25  Ss      pause   0xfffffe003ae4db20  sendmail
  755     1   755     0  Ss      select  0xfffffe001a96b140  sendmail
  726     1   726     0  Ss      nanslp  0xffffffff86ff77a0  cron
  646     1   646     0  Ss      select  0xfffffe001137d640  syslogd
  448     1   448     0  Ss      select  0xfffffe001a96b640  devd
  352     1   352    65  Ss      select  0xfffffe001a96b6c0  dhclient
  270     1   270     0  Ss      select  0xfffffe001a96bcc0  dhclient
  267     1   267     0  Ss      select  0xfffffe001137d840  dhclient
  102     0     0     0  DL      pftm    0xffffffff87731b30  [pf purge]
   21     0     0     0  DL      syncer  0xffffffff870ee800  [syncer]
   20     0     0     0  DL      vlruwt  0xfffffe001f831000  [vnlru]
   19     0     0     0  DL      (threaded)                  [bufdaemon]
100070                   D       qsleep  0xffffffff870ed8a0  [bufdaemon]
100071                   D       -       0xffffffff86a0af80  [bufspacedaemon-0]
100085                   D       sdflush 0xfffffe0010b198e8  [/ worker]
...
db> show all locks
Process 824 (syz-executor9159705) thread 0xfffffe001f953ac0 (100077)
exclusive sleep mutex swcr session lock (swcr session lock) r = 0 (0xfffffe003b381648) locked @ /root/freebsd/sys/opencrypto/cryptosoft.c:1625
db> show malloc
...
db> show uma
   ...
   mbuf_jumbo_page   4096    1088     942       14556       0     254    8314880        0
      malloc-16384  16384     105       0         105       0       1    1720320        0
       ...
       malloc-4096   4096     323       1         483       0       2    1327104        0
          BUF TRIE    144     169    8987         448       0      62    1318464        0
              pbuf   2624       0     264           0       0       2     692736        0
              mbuf    256    1347    1008       22570       0     254     602880        0
        malloc-512    512    1026      14        1034       0      30     532480        0
      malloc-65536  65536       6       1          37       0       1     458752        0
         vmem btag     56    7222      97        7222       0     254     409864        0
        RADIX NODE    144    2370     452       28853       0      62     406368        0
       ...
       malloc-2048   2048     136       6         160       0       8     290816        0
         ...
         VM OBJECT    264     907      53       11762       0      30     253440        0
        256 Bucket   2048      99      17        2848       0       8     237568        0
      ...
      malloc-65536  65536       3       0           3       0       1     196608        0
       malloc-4096   4096      40       3         829       0       2     176128        0
         UMA Zones    768     222       2         222       0      16     172032        0
        malloc-256    256     654       6        1196       0      62     168960        0
      ...
      malloc-32768  32768       2       2           4       0       1     131072        0
       ...
       S VFS Cache    104     995      97        1075       0     126     113568        0
         ...
         MAP ENTRY     96     776     316       37115       0     126     104832        0
          UMA Kegs    384     207       6         207       0      30      81792        0
         malloc-64     64     983      88       18242       0     254      68544        0
      ...
      malloc-32768  32768       2       0          54       0       1      65536        0
        ...
        128 Bucket   1024      44      15         688       0      16      60416        0
         filedesc0   1072      40      16         825       0       8      60032        0
        malloc-128    128     374      29        3643       0     126      51584        0
        malloc-384    384     111      19         150       0      30      49920        0
        malloc-128    128     348      24       28257       0     126      47616        0
            clpbuf   2624       0      15          29       0       5      39360        0
         32 Bucket    256      39     111         500       0      62      38400        0
             NAMEI   1024       0      36       12412       0      16      36864        0
             g_bio    384       0      90        4753       0      30      34560        0
        malloc-256    256     106      29         572       0      62      34560        0
           DIRHASH   1024      32       0          32       0      16      32768        0
      malloc-32768  32768       1       0           1       0       1      32768        0
      malloc-32768  32768       1       0           1       0       1      32768        0
      malloc-16384  16384       2       0           2       0       1      32768        0
           pcpu-64     64     455      57         455       0     254      32768        0
         64 Bucket    512      52      12         721       0      30      32768        0
            socket    944      20      12        1014       0     254      30208        0
       malloc-4096   4096       6       1           8       0       2      28672        0
            pcpu-8      8    3293     291        3323       0     254      28672        0
       malloc-4096   4096       4       2         274       0       2      24576        0
         malloc-64     64     260     118        3331       0     254      24192        0
       ...
       Mountpoints   2752       2       5           2       0       4      19264        0
        malloc-256    256      29      46         401       0      62      19200        0
              pipe    744       7      18         299       0      16      18600        0
      ...
      malloc-16384  16384       1       0          65       0       1      16384        0
       ...
       malloc-1024   1024       4       4           4       0      16       8192        0
        ...
        malloc-512    512       2      14           3       0      30       8192        0
         ...
         malloc-64     64      36      90          53       0     254       8064        0
     ...
     routing nhops    256       8      22          14       0      62       7680        0
       mbuf_packet    256       0      30         102       0     254       7680        0
        ...
        malloc-384    384       9      11          10       0      30       7680        0
        malloc-384    384       0      20         194       0      30       7680        0
        malloc-256    256       1      29         100       0      62       7680        0
 ...
 epoch_record pcpu    256       4      12           4       0      62       4096        0
      ...
      vtnet_tx_hdr     24       1     166        6956       0     254       4008        0
...
r0 = openat$crypto(0xffffffffffffff9c, &(0x7f0000000580), 0x0, 0x0)
r1 = openat$bpf(0xffffffffffffff9c, &(0x7f0000000000), 0x0, 0x0)
r2 = dup2(r0, r1)
ioctl$CIOCGSESSION2(r2, 0xc040636a, &(0x7f0000000040)={0x17, 0x25, 0x10, &(0x7f0000000100)="55b5000052c07da82ae6c22e7f00de07", 0x0, 0x0, 0x0, 0xfdfffffc})
ioctl$CIOCCRYPT(r2, 0xc0306367, &(0x7f00000000c0)={0x0, 0x2, 0x0, 0x3, &(0x7f0000000140), 0x0, 0x0, &(0x7f0000000340)})
...
int main(void)
{
  ...
  intptr_t res = 0;
  memcpy((void*)0x20000580, "/dev/crypto\000", 12);
  res = syscall(SYS_openat, 0xffffffffffffff9cul, 0x20000580ul, 0ul, 0ul);
  if (res != -1)
    r[0] = res;
  memcpy((void*)0x20000000, "/dev/bpf\000", 9);
  ...
  *(uint32_t*)0x20000044 = 0x25;
  *(uint32_t*)0x20000048 = 0x10;
  *(uint64_t*)0x20000050 = 0x20000100;
  memcpy((void*)0x20000100,
         "\x55\xb5\x00\x00\x52\xc0\x7d\xa8\x2a\xe6\xc2\x2e\x7f\x00\xde\x07",
         16);
  *(uint32_t*)0x20000058 = 0;
  *(uint64_t*)0x20000060 = 0;
  *(uint32_t*)0x20000068 = 0;
  *(uint32_t*)0x2000006c = 0xfdfffffc;
  *(uint32_t*)0x20000070 = 0;
  *(uint32_t*)0x20000074 = 0;
  *(uint32_t*)0x20000078 = 0;
  *(uint32_t*)0x2000007c = 0;
  syscall(SYS_ioctl, r[2], 0xc040636aul, 0x20000040ul);
  ...
  *(uint64_t*)0x200000e8 = 0x20000340;
  syscall(SYS_ioctl, r[2], 0xc0306367ul, 0x200000c0ul);
  return 0;
}
